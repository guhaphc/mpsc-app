...
export default async function TopicReader({params}:{params:Promise<{subjectId:string;topicId:string}>}){
 ...
 const {data:completion}=await supabase.from("study_material_progress").select("id").eq("user_id",profile.id).eq("topic_id",topicId).maybeSingle();
 return <div className="shell" style={{background:"var(--bg)"}}>
  ...
  <main style={{maxWidth:820,margin:"0 auto",padding:"28px 18px 70px"}}>
   ...
   <ReaderControls title={topic.title} notes={topic.notes} topicId={topicId} initialBookmarked={!!bookmark} initialCompleted={!!completion} contentBlocks={Array.isArray(topic.content_blocks)?topic.content_blocks:[]} subtopics={(subs||[]).map(s=>({id:s.id,title:s.title,content:s.content,content_blocks:Array.isArray(s.content_blocks)?s.content_blocks:[],important_keywords:Array.isArray(s.important_keywords)?s.important_keywords:[]}))}/>
