...
async function extractChunk(ai:any,ref:any,start:number,end:number,previousPath:string[]){
 ...
 const r=await ai.models.generateContent({model:process.env.GEMINI_SYLLABUS_MODEL||"gemini-2.5-flash-lite",contents:createUserContent([createPartFromUri(ref.uri,ref.mimeType),{text:prompt}]),config:{responseMimeType:"application/json",responseSchema:schema,maxOutputTokens:30000}});
