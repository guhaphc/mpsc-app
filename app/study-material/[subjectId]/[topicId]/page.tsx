...
export default async function TopicReader({params}:{params:Promise<{subjectId:string;topicId:string}>}){
 ...
 return <div className="shell" style={{background:"var(--bg)"}}>
  ...
  <main style={{maxWidth:820,margin:"0 auto",padding:"28px 18px 70px"}}>
   ...
   <ReaderControls title={topic.title} notes={topic.notes} topicId={topicId} initialBookmarked={!!bookmark} initialCompleted={!!completion} subtopics={(subs||[]).map(s=>({id:s.id,title:s.title,content:s.content,important_keywords:Array.isArray(s.important_keywords)?s.important_keywords:[]}))}/>
