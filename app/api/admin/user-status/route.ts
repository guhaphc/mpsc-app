...
export async function POST(request:Request){
 ...
 if(targetError||!target||target.role!=="student")return NextResponse.redirect(new URL("/admin?error=target",request.url),303);
 if(status==="blocked"){
   const {error:blockError}=await supabase.from("blocked_students").upsert({full_name:target.full_name,mobile:target.mobile,blocked_by:claimsData.claims.sub,reason:"Blocked by Admin"},{onConflict:"mobile"});
 ...
 await supabase.from("audit_logs").insert({actor_id:claimsData.claims.sub,action:status==="blocked"?"student_blocked":"account_status_changed",target_type:"profile",target_id:userId,metadata:{status}});
