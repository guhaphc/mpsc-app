...
export async function POST(req:Request){
 try{
  ...
  const {data:src,error:srcErr}=await s.from("ai_study_syllabus_sources").insert({name:sourceTitle,exam:"UPSC",academic_year:"2026-27",source_file_name:f.name,source_pages:84,status:"active",created_by:p.id}).select("id").single();if(srcErr||!src)throw new Error(srcErr?.message||"Could not create syllabus source.");
