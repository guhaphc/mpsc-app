...
export async function POST(request:Request){
 try{
  ...
  const prompt=`Translate the following published MPSC study note from English to Marathi. This is a faithful translation task, NOT summarization or rewriting. Preserve meaning, context, factual content, dates, names, terminology, headings, order, emphasis, and level of detail. Do not add facts or omit information. Keep proper names and specialist terms accurate; where useful for MPSC terminology, retain the English term in parentheses after its Marathi equivalent. Preserve the JSON structure exactly. Return ONLY valid JSON with title, notes, and subtopics (each with title and content). Source:\n${JSON.stringify({title,notes,subtopics})}`;
  ...
  return NextResponse.json({ok:true,result:translated});
