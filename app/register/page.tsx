...
export default function Register(){
 ...
 async function submit(e:FormEvent){e.preventDefault();setError("");if(form.password!==form.confirm){setError("Passwords do not match.");return;}setBusy(true);const supabase=createClient();const internalEmail=form.email.trim()||`${form.username.toLowerCase()}@mpsc-all-in-one.com`;const {error:signUpError}=await supabase.auth.signUp({email:internalEmail,password:form.password,options:{data:{full_name:form.full_name,mobile:form.mobile,username:form.username,role:form.role}}});if(signUpError){setError(signUpError.message);setBusy(false);return;}setDone(true);setBusy(false);}
 ...
 {fields.map(([key,label,type])=><div key={key}><label className="label">{label}</label><input className="input" type={type} placeholder={label.replace(" (optional)","")} value={form[key] as string} onChange={e=>setField(key,e.target.value)} required={key!=="email"}/></div>)}
